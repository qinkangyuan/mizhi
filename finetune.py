...
class ClassifierDataset(torch.utils.data.Dataset):
    def __init__(self, encodings, labels, device='gpu'):
...
class BertForSequenceClassificationWithFeatures(nn.Module):
    def __init__(self, num_labels, bert_model_name="bert_pretrain", freeze_bert=False):
...
def main():
    ...
    # train_dataset = ClassifierDataset(train_encodings, train_labels)
    # validation_dataset = ClassifierDataset(validation_encodings, validation_labels)
    ...
    model = BertForSequenceClassificationWithFeatures(num_labels=2, bert_model_name="bert_pretrain").to(device)
